<template>
    <p class="titleHeaderDiv">DIVISION</p>
    <form @submit.prevent="checkAnswer">
        <div id="theExpressionDiv">
            <p class="expressionDiv" style="margin-bottom 0">{{num1}} / {{num2}}</p>
        </div>
        <div v-if="numTimesWrong > 0" class="row wrongDiv">
    ...
    <div class="container" style="max-width: 30%; margin-top: 20px;">
        <div class="row">
            <div class="col-sm" :class="{ 'selectedDiv': difficultyLevel.at(0).enabled, 'divBtn': !difficultyLevel.at(0).enabled }" style="text-align: center" @click="difficulty(0)">
                <label class="btn" style="color:azure" id="0">
                    <!-- <input type="radio" name="options" id="easy" autocomplete="off" checked>  -->
                    Easy
                </label>
            </div>
            <div class="col-sm" :class="{ 'selectedDiv': difficultyLevel.at(1).enabled, 'divBtn': !difficultyLevel.at(1).enabled }" style="text-align: center;" @click="difficulty(1)">
                <label class="btn" style="color:azure" id="1">
                    <!-- <input type="radio" name="options" id="medium" autocomplete="off">  -->
                    Medium
                </label>
            </div>
            <div class="col-sm" :class="{ 'selectedDiv': difficultyLevel.at(2).enabled, 'divBtn': !difficultyLevel.at(2).enabled }" style="text-align: center;" @click="difficulty(2)">
    ...
    <div class="container">
        <div class="row">
            <div class="col">
                <p class="streakDiv">Streak: {{userStreak}} &nbsp; Best: {{allTimeBest}}&nbsp;</p>
            </div>
        </div>
    </div>
